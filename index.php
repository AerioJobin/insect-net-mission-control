        ...
        ?>
            ...
            <div class="container">
                ...
                <?php if ($_SESSION['role'] === 'admin'): ?>
                            <div class="user-chip admin-avatar-btn" id="adminChipBtn" title="Admin Settings" onclick="openAdminPanel()">
                                <div class="user-avatar"><?= strtoupper(substr($_SESSION['username'], 0, 1)) ?></div>
                                <div class="user-name"><?= htmlspecialchars($_SESSION['username']) ?></div>
                                <span class="user-role-badge"><?= htmlspecialchars($_SESSION['role']) ?></span>
                            </div>
    ...
    </html>SESSION['role'] === 'admin'): ?>
    <!-- ADMIN PANEL -->
    <div class="admin-panel-overlay" id="adminOverlay" onclick="closeAdminPanel()"></div>
    <div class="admin-panel" id="adminPanel">
        <div class="admin-panel-header">
            <div><p class="admin-panel-title">&#9881; Admin Settings</p><strong style="font-size:1em;">User Management</strong></div>
    ...
    <script>
    const CURRENT_USER = <?php echo json_encode(    <?php
    include('config.php');
    checkAccess('user'); // Anyone logged in can view
    // ── Config & Helpers ─────────────────────────────────────────
    $view = $_GET['view'] ?? 'landing';
    ...
    const INIT_USERS = <?php echo json_encode(array_map(fn($u) => ['username' => $u, 'role' => $users[$u]['role']], array_keys($users))); ?>;
    ...
    function showAdminMsg(msg,ok){const el=document.getElementById('adminMsg');el.textContent=msg;el.className='admin-msg '+(ok?'ok':'err');el.style.display='block';clearTimeout(el._t);el._t=setTimeout(()=>{el.style.display='none';},4500);}
    function renderUsers(users){const list=document.getElementById('adminUserList');list.innerHTML='';users.forEach(u=>list.appendChild(makeUserCard(u.username,u.role,false)));}
    function escH(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
    function makeUserCard(username,role,isNew){
        const card=document.createElement('div');card.className='user-card';const isSelf=username===CURRENT_USER;
        ...
        <div class="admin-field"><label class="admin-label">New Password <span style="font-weight:400;opacity:0.6;text-transform:none;letter-spacing:0;">${isNew?'(required)':'leave blank to keep current'}</span></label>
        ...
        <div class="user-card-actions"><button class="btn-save-user" onclick="saveUser(this)">&#128190; Save Changes</button>
        ${!isSelf?`<button class="btn-del-user" onclick="deleteUser(this,'${escH(username)}')">&#128465;</button>`:''}</div>`;
        return card;
    }
    function addUserCard(){const list=document.getElementById('adminUserList');const card=makeUserCard('','user',true);list.appendChild(card);card.querySelector('.admin-input').focus();card.scrollIntoView({behavior:'smooth',block:'nearest'});}
    async function saveUser(btn){
        const card=btn.closest('.user-card');const inputs=card.querySelectorAll('.admin-input');
        const newUsername=inputs[0].value.trim();const oldUsername=inputs[0].dataset.original||'';
        const newPassword=inputs[1].value;const newRole=card.querySelector('.admin-select').value;
        if(!newUsername){showAdminMsg('Username cannot be empty',false);return;}
        btn.disabled=true;btn.textContent='Saving...';
        try{const res=await fetch('admin_api.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save_user',old_username:oldUsername,username:newUsername,password:newPassword,role:newRole})});
        const d=await res.json();
        if(d.ok){showAdminMsg(d.message,true);inputs[0].dataset.original=newUsername;inputs[1].value='';card.querySelector('.user-card-name').textContent='&#128100; '+newUsername;card.querySelector('.user-card-role').textContent=newRole;
        if(oldUsername===CURRENT_USER){document.querySelectorAll('.user-name').forEach(el=>el.textContent=newUsername);document.querySelectorAll('.user-avatar').forEach(el=>el.textContent=newUsername[0].toUpperCase());}}
        else{showAdminMsg(d.error||'Save failed',false);}}catch(e){showAdminMsg('Network error',false);}
        btn.disabled=false;btn.textContent='&#128190; Save Changes';
    }
    async function deleteUser(btn,username){
        if(!confirm(`Delete user "${username}"?\nThis cannot be undone.`))return;
        btn.disabled=true;
        try{const res=await fetch('admin_api.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete_user',username})});
        const d=await res.json();
        if(d.ok){showAdminMsg(d.message,true);btn.closest('.user-card').remove();}
        else{showAdminMsg(d.error||'Delete failed',false);btn.disabled=false;}}catch(e){showAdminMsg('Network error',false);btn.disabled=false;}
    }
    </script>
    <?php endif; ?>
    </body>

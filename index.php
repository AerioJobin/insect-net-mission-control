    ...
    var INIT_USERS = <?php echo json_encode(array_map(function($u) use ($users){ return ["username"=>$u,"role"=>$users[$u]["role"]]; }, array_keys($users))); ?>;
    ...
    async function saveUser(btn){
        ...
        btn.disabled=true;btn.textContent="Saving...";
        try{
            var res=await fetch("admin_api.php",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"save_user",old_username:oldUsername,username:newUsername,password:newPassword,role:newRole})});
            var d=await res.json();
    ...
    async function deleteUser(btn,username){
        ...
        btn.disabled=true;
        try{
            var res=await fetch("admin_api.php",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"delete_user",username:username})});
            var d=await res.json();
